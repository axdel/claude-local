...
| scorecard | model-evaluation | Scorer (score_suite, Scorecard.write) | benchmark run script, cross-model comparison | single writer; totals derived from CaseResult economy records, never re-counted; filename via telemetry.slug_model_id | active |  |
